...
const rawText = fs.readFileSync('bakery.txt', 'utf8');

function generateId(index) {
  return "item-bk-" + index;
}
...
for (let line of lines) {
  ...
  if (!line.includes(':') && !line.startsWith('Deskripsi') && !line.startsWith('Porsi') && !line.startsWith('Dietary') && !line.startsWith('Sisi / Add') && !line.startsWith('Harga') && !line.startsWith('Penyajian')) {
    // This is a title!
    let name = line.replace(/^\d+\.\s*/, '');
    if (currentItem) items.push(currentItem);
    currentItem = {
      id: generateId(itemIndex++),
      categoryId: 'cat-3', // Bakery
      name: name,
      description: '',
...
fs.writeFileSync('bakery_dump.js', "export const GENERATED_BAKERY = [\n" + output + "\n];");
console.log('Done mapping ' + items.length + ' bakery items!');
